...
class CheckoutController extends Controller
{
    ...
    public function process()
    {
        ...
        try {
            ...
            if (!\App\Helpers\SecurityHelper::checkRateLimit($rateLimitKey, 10, 3600)) {
                ...
                if ($isAjax) {
                    $this->jsonResponse(['success' => false, 'message' => 'Too many checkout attempts. Please try again later.'], 429);
                } else {
            ...
            if (!empty($errors)) {
                if ($isAjax) {
                    $this->jsonResponse(['success' => false, 'message' => implode('<br>', $errors)], 400);
                } else {
            ...
            if ($orderId) {
                ...
                $this->handleSuccessfulOrder($orderId, $userId, $isAjax);
            } else {
                if ($isAjax) {
            ...
            if ($isAjax) {
                $this->jsonResponse(['success' => false, 'message' => 'An error occurred while processing your order: ' . $e->getMessage()], 500);
            } else {
     ...
     * The route now delegates to Payment\EsewaController@initiate.
    ...
    public function initiateEsewa($orderId)
    {
        ...
        try {
            ...
            // Get merchant code based on test/live mode
            $merchantCodeKey = $isTestMode ? 'test_merchant_id' : 'live_merchant_id';
            $merchantCode = $gatewayParams[$merchantCodeKey] ?? $gatewayParams['merchant_id'] ?? null;
            ...
            // Recalculate totals from order items using same logic as checkout
            // This ensures accuracy even if stored order values are incorrect
            ...
            $deliveryCharge = round($order['delivery_fee'] ?? 0, 2);
            ...
            // Calculate subtotal for eSewa (amount before tax and delivery)
            // eSewa expects: total_amount = amount + tax_amount + product_service_charge + product_delivery_charge
            $esewaSubtotal = max(0, round($totalAmount - $taxAmount - $serviceCharge - $deliveryCharge, 2));
            ...
            $secretKeyKey = $isTestMode ? 'test_secret_key' : 'live_secret_key';
            $secretKey = $gatewayParams[$secretKeyKey] ?? '';
            ...
            // eSewa calculates: total_amount = amount + tax_amount + product_service_charge + product_delivery_charge
            $paymentData = [
                ...
                'product_delivery_charge' => number_format($deliveryCharge, 2, '.', ''),
            ...
            $paymentUrl = $gatewayParams[$paymentUrlKey] ?? $defaultUrl;
    ...
    public function success($orderId)
    {
        try {
            ...
            // Handle digital products if order is paid
            ...
            if ($order['payment_status'] === 'paid') {
                ...
                // Refresh order data after processing (in case status changed)
                $order = $this->orderModel->getOrderWithItems($orderId);
            }

            // Check if order has digital products (regardless of payment status for UI)
            $digitalService = new \App\Services\DigitalProductService();
            $hasDigitalProducts = $digitalService->hasDigitalProducts($order);
            
            // Debug logging
            error_log("Checkout Success - Order #{$orderId}: hasDigitalProducts=" . ($hasDigitalProducts ? 'true' : 'false') . ", payment_status={$order['payment_status']}, items_count=" . count($order['items'] ?? []));

            $this->view('checkout/success', [
                'data' => ['order' => $order, 'hasDigitalProducts' => $hasDigitalProducts],
     ...
     */
    private function handleSuccessfulOrder($orderId, $userId, $isAjax = false)
    {
        ...
        if ($gateway['type'] === 'digital') {
            ...
            if ($isAjax) {
                $redirectUrl = $this->getDigitalPaymentRedirect($orderId, $gateway);
                $this->jsonResponse([
                    'success' => true,
            ...
            $successUrl = BASE_URL . '/checkout/success/' . $orderId;
    ...
    /**
     * Get digital payment redirect URL
     */
    private function getDigitalPaymentRedirect($orderId, $gateway)
    {
        switch ($gateway['slug']) {
            case 'khalti':
                return BASE_URL . '/checkout/khalti/' . $orderId;
            case 'esewa':
                return BASE_URL . '/checkout/esewa/' . $orderId;
            case 'mypay':
                return BASE_URL . '/checkout/mypay/' . $orderId;
            default:
                return BASE_URL . '/checkout/success/' . $orderId;
        }
    }
    ...
    private function processCouponValidation($code, $userId, $cartItems, $cartTotal = null)
    {
        ...
        // Validate seller coupon matching using CouponCheck
        if (!empty($coupon['seller_id'])) {
            $couponCheck = new \App\Helpers\CouponCheck();
            $mismatchProducts = [];
            ...
            foreach ($cartItems as $item) {
                ...
                if (!$couponCheck->isValid($coupon, $product)) {
                    $mismatchProducts[] = $product['product_name'] ?? 'Product';
                }
            }
            
            if (!empty($mismatchProducts)) {
                $productList = implode(', ', array_slice($mismatchProducts, 0, 3));
                if (count($mismatchProducts) > 3) {
                    $productList .= ' and more';
                }
                return [
                    'success' => false,
                    'message' => "This coupon is only valid for products from the same seller. Products: {$productList} do not match."
                ];
            }
        }
    ...
    private function initiateKhaltiPayment($paymentData, $gateway)
    {
        ...
        // Get secret key based on test/live mode
        $secretKeyKey = $isTestMode ? 'test_secret_key' : 'secret_key';
        $secretKey = $gatewayParams[$secretKeyKey] ?? $gatewayParams['secret_key'] ?? '';
        
        if (empty($secretKey)) {
            throw new Exception('Khalti secret key not configured for ' . ($isTestMode ? 'test' : 'live') . ' mode');
        }
            ...
            if (isset($responseData['status'])) {
                ...
                if ($order) {
                    ...
                    if ($status === 'Completed') {
                        ...
                        $digitalController = new \App\Controllers\Product\DigitalProductController();
                        $digitalController->processDigitalProductsAfterPayment($order['id']);
                        
                        // Send payment confirmation SMS
                        try {
                            $smsController = new \App\Controllers\Sms\SmsOrderController();
                            $smsController->sendPaymentConfirmationSms($order['id']);
                        } catch (\Exception $e) {
                            error_log("Checkout: Error sending payment confirmation SMS: " . $e->getMessage());
                        }
