...
class AuthController extends Controller
{
    ...
    public function processLogin()
    {
        ...
        if ($user) {
            ...
            // Prevent duplicate login with session locking
            if (!\App\Helpers\SessionLock::acquire($user['id'])) {
                $this->setFlash('error', 'Another login session is in progress. Please wait a moment.');
                $this->redirect('auth/login');
                return;
            }
            ...
            try {
                ...
                if (($maintenanceMode === 'true' || $maintenanceMode === true) && $userRole !== 'admin') {
                    \App\Helpers\SessionLock::release();
            ...
            if ($token) {
                ...
                // Release lock after successful login
                \App\Helpers\SessionLock::release();
            ...
            $this->recordLoginAttempt($ip, $phone, time());
    ...
    {
        try {
            $authUrl = $this->oauthService->getAuthorizationUrl('google');
            $this->externalRedirect($authUrl);
        } catch (Exception $e) {
            ...
            $this->setFlash('error', 'Google login failed: ' . $e->getMessage());
            $this->redirect('auth/login');
        }
    }
    ...
    public function googleCallback()
    {
        ...
        if ($error) {
            $this->setFlash('error', 'Google login cancelled: ' . htmlspecialchars($error));
            $this->redirect('auth/login');
            return;
        }

        if (!$code) {
            $this->setFlash('error', 'Google login failed: Missing authorization code.');
            $this->redirect('auth/login');
            return;
        }

        try {
            $user = $this->oauthService->handleCallback('google', $code, $state);
    ...
    private function authenticateUser($identifier, $password)
    {
        ...
        if (!$user) {
            ...
            try {
                ...
                if (!$staff && method_exists($staffModel, 'findByPhone')) {
                    $staff = call_user_func([$staffModel, 'findByPhone'], $identifier);
                }
