...
class AdminController extends Controller
{
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($errors)) {
                ...
                if ($productId) {
                    ...
                    $this->handleDigitalProduct($productId, $filtered['is_digital'] ?? 0);
     ...
     * @param bool $requireImages Whether to require images
     * @return array Array of validation errors (empty if valid)
     */
    private function validateProductData($data, $requireImages = true)
    {
        $errors = [];
    ...
    {
        try {
            $seoController = new \App\Controllers\Seo\SeoController();
            ...
            if ($result) {
                ...
                if ($status === 'delivered' && $oldStatus !== 'delivered') {
                    ...
                    try {
                        ...
                        $payoutController->processSellerPayout($order['id']);
                    } catch (\Exception $e) {
                        error_log('AdminController: Error processing seller payout: ' . $e->getMessage());
                    }
                }
                ...
                if ($status === 'cancelled' && $oldStatus !== 'cancelled') {
                    ...
                    // Notify sellers about cancellation using NotificationSellerController
                    try {
                        $sellerNotificationController = new \App\Controllers\Notification\NotificationSellerController();
                        $sellerNotificationController->notifyOrderCancelled($order['id']);
                    } catch (\Exception $e) {
                        error_log('AdminController: Order cancellation notification error: ' . $e->getMessage());
                    }
                }
                ...
                if ($oldStatus !== $status) {
                    ...
                    // Notify seller about order status change using NotificationSellerController
                    try {
                        $sellerNotificationController = new \App\Controllers\Notification\NotificationSellerController();
                        $sellerNotificationController->notifyOrderStatusChange($id, $oldStatus, $status);
                    } catch (Exception $sellerNotifException) {
                        error_log("AdminController: Seller notification error for order #{$id}: " . $sellerNotifException->getMessage());
                    ...
                    try {
                        ...
                        $customerNotificationController->notifyOrderStatusChange($order['user_id'], $id, $oldStatus, $status);
                    } catch (Exception $customerNotifException) {
                        error_log("Customer notification error for order #{$id}: " . $customerNotifException->getMessage());
                    }
                    
                    // Notify courier if order is assigned
                    if (!empty($order['curior_id'])) {
                        try {
                            $courierNotificationController = new \App\Controllers\Notification\NotificationCourierController();
                            $courierNotificationController->notifyOrderStatusUpdate($order['curior_id'], $id, $status);
                        } catch (Exception $courierNotifException) {
                            error_log("Courier notification error for order #{$id}: " . $courierNotifException->getMessage());
                        }
                    }
                }
            ...
            if ($result) {
                ...
                if ($oldPaymentStatus !== $paymentStatus && $paymentStatus === 'paid') {
                    try {
                        ...
                        $smsResult = $smsController->sendPaymentConfirmationSms($id);
                        
                        if ($smsResult) {
                            error_log("SMS notification sent successfully for payment confirmation on order #{$id}");
                        } else {
                            error_log("SMS notification failed for payment confirmation on order #{$id}");
                        }
                    } catch (Exception $smsException) {
                        error_log("SMS notification error for payment confirmation on order #{$id}: " . $smsException->getMessage());
                        // Don't fail the payment update if SMS fails
                    }
                    
                    // Handle digital products after payment
                    try {
                        $digitalController = new \App\Controllers\Product\DigitalProductController();
                        $digitalController->processDigitalProductsAfterPayment($id);
                    } catch (Exception $digitalException) {
                        error_log("Digital product processing error for order #{$id}: " . $digitalException->getMessage());
                    }
                }
                
                // Notify customer about payment status change
                if ($oldPaymentStatus !== $paymentStatus && !empty($order['user_id'])) {
                    try {
                        $customerNotificationController = new \App\Controllers\Notification\NotificationCustomerController();
                        $customerNotificationController->notifyPaymentStatusChange($order['user_id'], $id, $oldPaymentStatus, $paymentStatus);
                    } catch (Exception $customerNotifException) {
                        error_log("Customer payment notification error for order #{$id}: " . $customerNotifException->getMessage());
                    }
                }
