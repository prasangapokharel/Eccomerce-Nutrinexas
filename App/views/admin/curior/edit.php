...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">Edit Curior</h1>
            <p class="mt-1 text-sm text-gray-500">Update curior information</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="<?= \App\Core\View::url('admin/curior/edit/' . $curior['id']) ?>" method="POST" id="curiorForm">
            <!-- Form Header -->
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                <h2 class="text-lg font-semibold text-gray-900">Curior Information</h2>
                <p class="text-sm text-gray-600 mt-1">Update the curior account details</p>
            </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="name" 
                               name="name" 
                               value="<?= htmlspecialchars($data['name'] ?? $curior['name']) ?>"
                    ...
                    <div>
                        ...
                        <input type="email" 
                               id="email" 
                               name="email" 
                               value="<?= htmlspecialchars($data['email'] ?? $curior['email']) ?>"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary text-sm"
                    ...
                    <div>
                        ...
                        <input type="tel" 
                               id="phone" 
                               name="phone" 
                               value="<?= htmlspecialchars($data['phone'] ?? $curior['phone']) ?>"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary text-sm"
                    ...
                    <div>
                        ...
                        <textarea id="address" 
                                  ...
                                  placeholder="Enter curior address"><?= htmlspecialchars($data['address'] ?? $curior['address']) ?></textarea>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Minimum 6 characters (leave blank to keep current password)</p>
                    ...
                    <div>
                        ...
                        <select id="status" 
                                name="status" 
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary text-sm">
                            <option value="active" <?= (isset($data['status']) ? $data['status'] : $curior['status']) === 'active' ? 'selected' : '' ?>>Active</option>
                            <option value="inactive" <?= (isset($data['status']) ? $data['status'] : $curior['status']) === 'inactive' ? 'selected' : '' ?>>Inactive</option>
                        </select>
                        <p class="text-xs text-gray-500 mt-1">Set the status for this curior</p>
                    </div>
                </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-sm text-gray-900"><?= date('M d, Y - H:i', strtotime($curior['created_at'])) ?></p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Last Updated</label>
                                <p class="text-sm text-gray-900"><?= date('M d, Y - H:i', strtotime($curior['updated_at'])) ?></p>
                            </div>
                        </div>
                    </div>
                </div>
