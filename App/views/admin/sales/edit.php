        ...
        <form method="POST" action="<?= \App\Core\View::url('admin/sales/update/' . $sale['id']) ?>" class="space-y-6">
            ...
            <div>
                <label for="sale_percent" class="block text-sm font-medium text-gray-700 mb-2">
                    Sale Percentage <span class="text-red-500">*</span>
                </label>
                <div class="relative">
                    <input type="number" name="sale_percent" id="sale_percent" required
                           min="1" max="99" step="0.1"
                           value="<?= htmlspecialchars($sale['sale_percent'] ?? $sale['discount_percent'] ?? '') ?>"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
                           placeholder="10">
                    <span class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500">%</span>
                </div>
                <p class="mt-1 text-xs text-gray-500">Enter sale percentage (1-99%). Products with sale='on' will show this discount.</p>
            </div>
