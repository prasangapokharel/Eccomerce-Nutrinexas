<?php
/**
 * Staff Authentication API v1
 * Clean, minimal API for staff login with bearer token
 * 
 * @package NutriNexus\Api\V1\Auth\Staff
 * @version 1.0.0
 */
...
/**
 * Staff Authentication API v1 Controller
 */
class StaffAuthApiV1
{
    private $staffModel;
    ...
    public function login()
    {
        try {
            ...
            $token = $this->generateBearerToken($staff['id'], $staff['email']);
            ...
            error_log("Staff auth API error: " . $e->getMessage());
     ...
     * Generate bearer token
     */
    private function generateBearerToken($staffId, $email)
    {
        $payload = [
            'staff_id' => $staffId,
            'email' => $email,
...
// Route to staff auth endpoints
if (count($pathSegments) >= 5 && 
    $pathSegments[0] === 'api' && 
    $pathSegments[1] === 'v1' && 
    $pathSegments[2] === 'auth' && 
    $pathSegments[3] === 'staff') {
    
    $action = $pathSegments[4] ?? '';
    $controller = new StaffAuthApiV1();
